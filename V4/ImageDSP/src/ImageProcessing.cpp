...
void imageProcessingFun(const QString& progName, QImage& outImgs, const QImage& inImgs, const QVector<double>& params)
{
	...
	{	
		...
		performDCT(Y_buff, X_SIZE, Y_SIZE, params[0]);
		
	}
	else if (progName == "DCT + IDCT") 
	{
		/* Perform NxN DCT + IDCT with block size 8 */
		performDCTandIDCT(Y_buff, X_SIZE, Y_SIZE, params[0]);
	}
	...
	else if (progName == "Mask DCT")
	{
		...
		performMaskDCTCoeffs(Y_buff, X_SIZE, Y_SIZE, params[0], floor(params[1]*params[0]*params[0]/100));
	}
