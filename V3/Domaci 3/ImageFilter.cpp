...
void performSobelEdgeDetection(uchar input[], int xSize, int ySize, double threshold)
{
	...
	// Extend image
	// TO DO
    uchar *extendedImage = new uchar[(xSize + 2 * delta) * (ySize + 2 * delta)];
	...
	for (int i = 0; i < xSize; i++)
	{
		for (int j = 0; j < ySize; j++)
		{
			...
			// TO DO
            if(sqrt(pow(hAccum, 2) + pow(vAccum, 2)) < threshold) input[j * xSize + i] = 0;
            else input[j * xSize + i] = 255;
...
void performCannyEdgeDetection(uchar input[], int xSize, int ySize, double threshold1, double threshold2)
{
	...
	for (int i = 0; i < xSize; i++)
	{
		for (int j = 0; j < ySize; j++)
		{
			...
			// TO DO
            edgeMagnitude[j * xSize + i] = (sqrt(pow(hAccum, 2) + pow(vAccum, 2)));
		...
		for (int j = 0; j < ySize; j++)
		{
			...
			else
			{
				...
				case 0:
                    // TO DO
                    if (j > 0)
                        sused1 = edgeMagnitude[(j - 1) * xSize + i];
                    if (j < ySize - 1)
                        sused2 = edgeMagnitude[(j + 1) * xSize + i];
                    if((sused1 > threshold2) || (sused2 > threshold2)) input[j * xSize + i] = 255;
					break;
				case 45:
					// TO DO
                    if (j > 0 && i > 0)
                        sused1 = edgeMagnitude[(j - 1) * xSize + i - 1];
                    if (j < ySize - 1 && i < xSize - 1)
                        sused2 = edgeMagnitude[(j + 1) * xSize + i + 1];
                    if((sused1 > threshold2) || (sused2 > threshold2)) input[j * xSize + i] = 255;
					break;
				case 90:
					// TO DO
                    if (i > 0)
                        sused1 = edgeMagnitude[(j)*xSize + i - 1];
                    if (i < xSize - 1)
                        sused2 = edgeMagnitude[(j)*xSize + i + 1];
                    if((sused1 > threshold2) || (sused2 > threshold2)) input[j * xSize + i] = 255;
					break;
				case 135:
					// TO DO
