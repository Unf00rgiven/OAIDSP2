...
std::vector<PossiblePlate> detectCharsInPlates(std::vector<PossiblePlate> &vectorOfPossiblePlates) {
    ...
    for (auto &possiblePlate : vectorOfPossiblePlates) {            // for each possible plate, this is a big for loop that takes up most of the function
      ...
      //  possiblePlate.strChars = recognizeCharsInPlate(possiblePlate.imgThresh, possiblePlate.imgPlate, longestVectorOfMatchingCharsInPlate);
        possiblePlate.strChars = recognizeCharsInPlate(possiblePlate.imgThresh, possiblePlate.imgPlate, longestVectorOfMatchingCharsInPlate);
#ifdef SHOW_STEPS
...
std::string recognizeCharsInPlate(cv::Mat& imgThresh, cv::Mat& imgPlate, std::vector<PossibleChar>& vectorOfMatchingChars) {
    std::string strChars;               // this will be the return value, the chars in the lic plate


    tesseract::TessBaseAPI* api = new tesseract::TessBaseAPI();
    // Initialize tesseract-ocr with English, without specifying tessdata path
    if (api->Init(NULL, "leu")) {
        fprintf(stderr, "Could not initialize tesseract.\n");
        return strChars;
    }

    cv::Mat imgThresh2(imgThresh.size(), imgThresh.type(), cv::Scalar(0));

    for (PossibleChar& pch : vectorOfMatchingChars)
    {
        int width = pch.boundingRect.width;
        int height = pch.boundingRect.height;
        int x = pch.boundingRect.x;
        int y = pch.boundingRect.y;
        ...
        for (int i = 0; i < imgThresh.rows; i++)
        {
            for (int j = 0; j < imgThresh.cols; j++)
            {
                ...
                {
                    imgThresh2.at<uint8_t>(i, j) = imgThresh.at<uint8_t>(i, j);
                }
    ...
    api->SetImage(imgThresh2.data, imgThresh2.cols, imgThresh2.rows, 1, 1 * imgThresh2.cols);


    char* outtext = api->GetUTF8Text();
    strChars = std::string(outtext);        // append current char to full string
   
#ifdef SHOW_STEPS
    cv::imshow("10", imgThresh2);
#endif //SHOW_STEPS
